...
find_nvidia_icd() {
  ...
  for candidate in \
    ...
    /usr/share/vulkan/icd.d/*nvidia*.json; do
    if [[ -f "$candidate" ]]; then
      printf '%s\n' "$candidate"
      return 0
    fi
...
NVIDIA_ICD="${VK_ICD_FILENAMES:-}"
if [[ -z "$NVIDIA_ICD" ]]; then
  NVIDIA_ICD="$(find_nvidia_icd || true)"
fi
if [[ -z "$NVIDIA_ICD" ]]; then
  fatal "NVIDIA Vulkan ICD not found; set VK_ICD_FILENAMES=/path/to/nvidia_icd.json"
fi
...
log "VK_ICD_FILENAMES=$NVIDIA_ICD"
log "XR_OUT=$XR_OUT"
