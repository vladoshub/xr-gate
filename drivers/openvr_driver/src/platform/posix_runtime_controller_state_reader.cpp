...
std::string normalize_posix_shm_name(const std::string& name) {
  if (name.empty()) return "/runtime_controller_state";
...
class PosixRuntimeControllerStateReader final : public IRuntimeControllerStateReader {
 public:
  ...
  ~PosixRuntimeControllerStateReader() override { close_mapping(); }
  ...
  bool read_latest(RuntimeControllerStateSample& out, std::string* error) override {
    ...
    if (latest == 0) {
      if (error) *error = "runtime controller state ring has no committed frames yet";
      return false;
    }

    if (header->slot_count == 0 || header->slot_stride == 0 || header->header_size == 0) {
      if (error) *error = "runtime controller state ring header is invalid";
      return false;
    }
    ...
    if (payload_offset + sizeof(xr_runtime::RuntimeControllerStateFrameV1) > mapped_size_) {
      if (error) *error = "runtime controller state slot is outside mapped SHM size";
      return false;
    }
    ...
    if (error) *error = "runtime controller state slot changed while reading";
    return false;
  }
 ...
 private:
  ...
  bool validate_header(std::string* error) const {
    if (mapped_size_ < sizeof(xr_runtime::RingHeaderV1)) {
      if (error) *error = "runtime controller state SHM is smaller than ring header";
      return false;
    }
    ...
    if (std::memcmp(h->magic, expected, sizeof(expected)) != 0) {
      if (error) *error = "runtime controller state SHM magic mismatch";
      return false;
    }
    if (h->payload_size != sizeof(xr_runtime::RuntimeControllerStateFrameV1)) {
      if (error) *error = "runtime controller state payload size mismatch";
      return false;
    }
    if (h->slot_header_size != sizeof(xr_runtime::RingSlotHeaderV1)) {
      if (error) *error = "runtime controller state slot header size mismatch";
      return false;
    }
    return true;
  }
