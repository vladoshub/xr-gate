...
std::string normalize_posix_shm_name(const std::string& name) {
  if (name.empty()) return "/hand_tracking";
...
class PosixRuntimeHandReader final : public IRuntimeHandReader {
 public:
  ...
  ~PosixRuntimeHandReader() override { close_mapping(); }

  bool read_latest(RuntimeHandSample& out, std::string* error) override {
    if (!ensure_mapping(error)) return false;

    auto* header = reinterpret_cast<const xr_tracking::TrackingRingHeaderV1*>(data_);
    const uint64_t latest = header->latest_sequence;
    if (latest == 0) {
      if (error) *error = "hand tracking ring has no committed frames yet";
      return false;
    }

    if (header->slot_count == 0 || header->slot_stride == 0 || header->header_size == 0) {
      if (error) *error = "hand tracking ring header is invalid";
      return false;
    }
    ...
    if (payload_offset + sizeof(xr_tracking::HandTrackingFrameF32V2) > mapped_size_) {
      if (error) *error = "hand tracking slot is outside mapped SHM size";
      return false;
    }

    const auto* slot = reinterpret_cast<const xr_tracking::TrackingSlotHeaderV1*>(data_ + slot_offset);
    xr_tracking::HandTrackingFrameF32V2 frame{};

    for (int attempt = 0; attempt < 3; ++attempt) {
      const uint64_t begin = slot->seq_begin;
      std::atomic_thread_fence(std::memory_order_acquire);
      std::memcpy(&frame, data_ + payload_offset, sizeof(frame));
      std::atomic_thread_fence(std::memory_order_acquire);
      const uint64_t end = slot->seq_end;

      if (begin == end && (begin % 2u) == 0u && begin != 0 && frame.sequence * 2u == begin) {
        out.frame = frame;
    ...
    if (error) *error = "hand tracking slot changed while reading";
    return false;
  }
 ...
 private:
  ...
  bool validate_header(std::string* error) const {
    if (mapped_size_ < sizeof(xr_tracking::TrackingRingHeaderV1)) {
      if (error) *error = "hand tracking SHM is smaller than ring header";
      return false;
    }
    const auto* h = reinterpret_cast<const xr_tracking::TrackingRingHeaderV1*>(data_);
    const char expected[8] = {'H', 'T', 'R', 'K', 'R', 'G', '1', '\0'};
    if (std::memcmp(h->magic, expected, sizeof(expected)) != 0) {
      if (error) *error = "hand tracking SHM magic mismatch";
      return false;
    }
    if (h->payload_size != sizeof(xr_tracking::HandTrackingFrameF32V2)) {
      if (error) *error = "hand tracking payload size mismatch; expected HAND_TRACKING_21_JOINT_F32_V2";
      return false;
    }
    if (h->slot_header_size != sizeof(xr_tracking::TrackingSlotHeaderV1)) {
      if (error) *error = "hand tracking slot header size mismatch";
      return false;
    }
    return true;
  }
