...
install_monado_legal_files() {
  local license_dir="$BIN_DIR/licenses"
  local resolved_commit

  [[ -d "$MONADO_DIR/LICENSES" ]] || fail "Monado LICENSES directory not found: $MONADO_DIR/LICENSES"
  rm -rf "$license_dir"
  mkdir -p "$license_dir/monado"
  cp -a "$MONADO_DIR/LICENSES" "$license_dir/monado/LICENSES"

  resolved_commit="$(git -C "$MONADO_DIR" rev-parse HEAD)"
  cat > "$license_dir/SOURCE_INFO.txt" <<EOF_MONADO_SOURCE
Component: Monado runtime with XR Gate driver integration
License: per-file SPDX licenses; complete upstream LICENSES directory included
Upstream repository: $MONADO_REPO
Requested ref: $MONADO_REF
Resolved commit: $resolved_commit
XR Gate driver/integration source: project MIT license unless an explicit SPDX header states otherwise
The Monado checkout is patched before building; upstream does not endorse the modified binary.
EOF_MONADO_SOURCE
  log "installed Monado legal bundle: $license_dir"
}
