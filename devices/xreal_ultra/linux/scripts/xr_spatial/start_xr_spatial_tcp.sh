...
SCRIPT="$XR_BIN_ROOT/scripts/backends/xr_spatial/start_xr_spatial_tcp.sh"
if [[ ! -x "$SCRIPT" ]]; then
  SCRIPT="$XR_ROOT_PROJECT/backends/xr_spatial/scripts/linux/start_xr_spatial_tcp.sh"
fi
exec "$SCRIPT" "$@"
