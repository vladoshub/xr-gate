...
usage() {
  cat <<'USAGE'
Install Ubuntu 24.04 runtime dependencies for an already built XR Gate runtime
package.

This common installer owns system packages, the package-local thin Python venv
and runtime checks. Device-specific permissions/udev setup is delegated through
XR_RUNTIME_DEVICE_ACCESS_SCRIPT.

Usage:
  devices/common/linux/scripts/runtime/install_runtime_deps_ubuntu24.sh [options]

Options:
  --no-apt       Do not install apt packages.
  --no-groups    Ask the device access hook not to modify user groups.
  --no-udev      Ask the device access hook not to install udev rules.
  --no-device-access  Skip the device-specific access hook entirely.
...
Environment:
  XR_TARGET_DEVICE                Device target name. Default: generic.
  XR_PACKAGE_ROOT                 Runtime package/source root. Auto-detected.
  XR_RUNTIME_DEVICE_ACCESS_SCRIPT Optional executable device hook.
  XR_RECREATE_VENV                If 1, recreate bin/python-runtime/venv.
USAGE
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-device-access) INSTALL_DEVICE_ACCESS=0 ;;
...
XR_TARGET_DEVICE="${XR_TARGET_DEVICE:-generic}"
...
if [[ "$INSTALL_APT" == "1" ]]; then
  ...
  runtime_packages=(
    ...
    # Generic camera/HID/USB access for capture_service_cpp and device tools.
...
if [[ "$INSTALL_DEVICE_ACCESS" == "1" && -n "${XR_RUNTIME_DEVICE_ACCESS_SCRIPT:-}" ]]; then
  if [[ ! -x "$XR_RUNTIME_DEVICE_ACCESS_SCRIPT" ]]; then
    fatal "device access hook is not executable: $XR_RUNTIME_DEVICE_ACCESS_SCRIPT"
  fi
  log "run device access hook: $XR_RUNTIME_DEVICE_ACCESS_SCRIPT"
  XR_RUNTIME_INSTALL_GROUPS="$INSTALL_GROUPS" \
  XR_RUNTIME_INSTALL_UDEV="$INSTALL_UDEV" \
  XR_RUNTIME_SUDO_COMMAND="${SUDO[*]:-}" \
    "$XR_RUNTIME_DEVICE_ACCESS_SCRIPT"
elif [[ "$INSTALL_DEVICE_ACCESS" == "1" ]]; then
  log "no device access hook configured; skip device-specific permissions"
fi
...
Notes:
  - A device access hook may require re-login/reboot after group or udev changes.
