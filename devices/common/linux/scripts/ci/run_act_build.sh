...
log() { echo "[run_act_build] $*" >&2; }
warn() { echo "[run_act_build][WARN] $*" >&2; }
fatal() { echo "[run_act_build][ERROR] $*" >&2; exit 1; }

usage() {
  cat <<'USAGE'
Run an XR Gate GitHub Actions build workflow locally through nektos/act.
...
By default this script:
  ...
  - runs the workflow selected by WORKFLOW/XR_ACT_DEFAULT_WORKFLOW through act;
  - writes act artifacts to /tmp/xr_tracking_act_artifacts;
  - writes logs to /tmp/xr_tracking_ci_logs.

Usage:
  devices/common/linux/scripts/ci/run_act_build.sh [options]
...
WORKFLOW="${WORKFLOW:-${XR_ACT_DEFAULT_WORKFLOW:-}}"
...
ACT_LOG_STEM="${XR_ACT_LOG_STEM:-xr_gate}"
LOG_FILE="$ACT_LOG_DIR/act_${ACT_LOG_STEM}_$(date +%Y%m%d_%H%M%S).log"
...
Artifacts were written below:
  $ACT_ARTIFACT_DIR

Expected device artifact hint:
  ${XR_ACT_MAIN_ARTIFACT_HINT:-configured by the selected workflow}

Expected optional model artifact hint:
  ${XR_ACT_MODELS_ARTIFACT_HINT:-hand-tracking-models-mercury.tar.gz}
EOF2
