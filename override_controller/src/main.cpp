...
bool fingerprint_same_strict_input_device(const DeviceFingerprint& wanted, const DeviceFingerprint& candidate) {
  ...
  // can change after reconnect/reboot, while uniq/by-id/by-path/phys may still
  // identify the same physical input node. Treat any strong concrete identity
  // match as the same device; if none matches, it is not safe to match.
  if (!wanted.uniq.empty() && !candidate.uniq.empty() && wanted.uniq == candidate.uniq) {
    return true;
  }
  if (!wanted.by_id_path.empty() && !candidate.by_id_path.empty() && wanted.by_id_path == candidate.by_id_path) {
    return true;
  }
  if (!wanted.by_path.empty() && !candidate.by_path.empty() && wanted.by_path == candidate.by_path) {
    return true;
  }
  if (!wanted.phys.empty() && !candidate.phys.empty() && wanted.phys == candidate.phys) {
    return true;
  }
  if (!wanted.event_path.empty() && !candidate.event_path.empty() && wanted.event_path == candidate.event_path) {
    return true;
  }
